                ...
                <div class="sidebar-menu-list">
                    @foreach ($sidebarSchools as $school)
                        @php
                            ...
                            $isOpenGroup = ($activeGroup ?? null) === $groupKey;
                        ...
                        <div class="sidebar-group" data-group="{{ $groupKey }}">
                            <button class="sidebar-menu-btn">
                                ...
                                <svg class="sb-arrow{{ $isOpenGroup ? ' open' : '' }}" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                    stroke-linecap="round" stroke-linejoin="round">
                                    <path d="m9 18 6-6-6-6"></path>
                                </svg>
                            </button>
                            <div class="sidebar-sublist{{ $isOpenGroup ? ' is-open' : ' is-hidden' }}">
                                @foreach ($school->courses as $sidebarCourse)
                                    <a class="sidebar-sublink{{ ($activeCourseSlug ?? null) === $sidebarCourse->slug ? ' active' : '' }}"
                                        href="{{ course_url($school->slug, $sidebarCourse->slug) }}">{{ $sidebarCourse->coursename }}</a>
                                @endforeach
                            </div>
                        </div>
                    @endforeach
