@php
    $waPhone = $phone ?? config('pokerexams.whatsapp.phone', '15107718152');
    $waMessage = $message ?? config('pokerexams.whatsapp.message', 'Hello');
    $waUrl = 'https://api.whatsapp.com/send/?phone=' . rawurlencode($waPhone)
        . '&text=' . rawurlencode($waMessage)
        . '&type=phone_number&app_absent=0';
@endphp

<a href="{{ $waUrl }}" class="wa-floating-widget" target="_blank" rel="noopener noreferrer"
    aria-label="Chat on WhatsApp">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width="30" height="30" aria-hidden="true">
